...
version_info('SEE v1.0.5 (2024-03-23)').
...
:- dynamic('<http://www.w3.org/2000/10/swap/lingua#backimplication>'/2).
...
gre(Argus) :-
    ...
    assertz(implies((
            '<http://www.w3.org/2000/10/swap/lingua#backimplication>'(B, A),
            findvars([A, B], V, alpha),
            list_to_set(V, U),
            makevars([A, B, U], [Q, I, X], beta(U)),
            zip_list(U, X, W),
    ...
    assertz(implies((
            ...
            conj_list(G, L),
            list_to_set(L, B),
            select('<http://www.w3.org/2000/10/swap/lingua#nand>'(Z, H), B, K),
            getlist(Z, Zl),
            is_list(Zl),
            is_graph(H),
            conj_list(H, M),
            list_to_set(M, T),
            select('<http://www.w3.org/2000/10/swap/lingua#nand>'(W, O), T, N),
            getlist(W, Wl),
            is_list(Wl),
            is_graph(O),
            (   conj_list(O, D),
            ...
            ;   length(K, I),
                I > 1,
                conj_list(F, N),
                conj_list(C, ['<http://www.w3.org/2000/10/swap/lingua#nand>'([], F)|K])
            ),
            findvars(H, R, beta),
            intersection(Zl, R, X),
            findvars(O, S, beta),
            intersection(Wl, S, Y),
            append([Vl, X, Y], U)
            ), '<http://www.w3.org/2000/10/swap/lingua#nand>'(U, C))),
    ...
    assertz(implies((
            ...
            length(B, D),
            memberchk(E, [0, 2, D]),
            '<http://www.w3.org/2000/10/swap/lingua#nand>'(W, F),
            getlist(W, Wl),
            is_list(Wl),
            is_graph(F),
            conj_list(F, K),
            ...
            is_list(Ul),
            is_graph(C),
            (   select('<http://www.w3.org/2000/10/swap/lingua#nand>'(Z, Q), B, A),
                M = ['<http://www.w3.org/2000/10/swap/lingua#nand>'(Ul, C)|A],
                conj_list(Q, R),
                memberchk(P, R)
            ;   select(Q, B, A),
                M = [P|A],
    ...
    assertz(implies((
            ...
            is_graph(G),
            conj_list(G, [G]),
            (   G = '<http://www.w3.org/2000/10/swap/lingua#nand>'(Z, H)
            ->  true
            ;   Z = [],
                H = '<http://www.w3.org/2000/10/swap/lingua#nand>'([], G)
            ),
            findvars(H, R, beta),
            intersection(Z, R, X),
            conj_list(H, B),
            ...
            ), '<http://www.w3.org/2000/10/swap/lingua#backimplication>'(Q, I))),
    % blow inference fuse
    assertz(implies((
            '<http://www.w3.org/2000/10/swap/lingua#nand>'(V, G),
            call((
                ...
                (   H = '<http://www.w3.org/2000/10/swap/lingua#nand>'(_, C)
                ->  I = '<http://www.w3.org/2000/10/swap/lingua#nand>'(_, C)
                ;   I = H
                )
            )),
            J,
            '<http://www.w3.org/2000/10/swap/lingua#nand>'(_, I)
            ), false)),
    % set engine values
    findall(Sc,
        (   scope(Sc)
